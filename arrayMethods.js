...
const evenNumbers  = mixedNumbers.filter(findEvenNumbers)
...
const totalPopulation   = populations.reduce(reducer)
...
const myStrongest  = monstersInYourPocket.filter( el => {  return el.CP > 200});
...
const ordersWithTax  = orders.map(item => {
  let orderTotal = item.price*(1+item.tax);
    return orderTotal
})
//console.log(ordersWithTax)

orderTotals = ordersWithTax.reduce(reducer)
...
// CODE HERE
  ...
  const bobsOrders = purchases.filter(function (e) {
    return e.owner ==="Bob";
  })
// console.log(bobsOrders)

const bobsPrices = bobsOrders.map(e => e.price)
//console.log(bobsPrices)

const bobsTotal  = bobsPrices.reduce(reducer)
